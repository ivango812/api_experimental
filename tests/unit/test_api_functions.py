...
import scoring
from .mock import StorageDBLayerMockMem
...
class TestApiFunctionsSuite(unittest.TestCase):
    ...
    def setUp(self):
        self.storage_db_layer_mock = StorageDBLayerMockMem()
        scoring.gen_interests(self.storage_db_layer_mock, 0, 10)
    ...
    def test_get_score(self, arguments):
        ...
        score = scoring.get_score(self.storage_db_layer_mock, **arguments)
        self.assertEqual(result, score, "get_score({}) wrong result".format(arguments))

    def test_get_interests(self):
        for i in range(0, 10):
            res = scoring.get_interests(self.storage_db_layer_mock, i)
